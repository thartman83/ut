...
(ert-deftest test-ut-new-test-suite ()
  (with-temporary-dir
   ...
   (ut-new-conf ".tests" "foo" (f-expand "./") (f-expand "./tests") 'echo)
   (should (= (ut-test-suite-count) 0))
   (make-directory "tests/foo")
   ...
   (should (string= (f-join (ut-test-dir) (ut-test-suite-test-dir (first (ut-test-suites))))
                    (f-expand "./tests/foo")))
...
(ert-deftest test-errors-on-add-and-del-test-suite ()
  (with-temporary-dir
   ...
   (ut-new-test-suite "foo" (f-expand "./tests/foo") 'echo)
   (should-error (ut-new-test-suite "foo" (f-expand "./tests/foo") 'echo)
