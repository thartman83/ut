...
** TODO adding a test asks for the framework, which it should just inherit from the conf
** TODO adding AC_CONFIG_FILES for the test directory doesn't appear to be automatic for some reason
...
** TODO Wrong path for cunit-sexp-outputer include and lib
