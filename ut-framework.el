...
https//github.com/flycheck/"
	...
	(let ((build-command (plist-get properties :build-command))
		...
		(unless (or (null build-filter) (functionp (symbol-function (eval build-filter))))
		...
		(unless (functionp (symbol-function (eval run-filter)))
			(error "Run filter must be a function"))
		(unless (or (null new-test-suite-hook)
								(functionp (symbol-function (eval new-test-suite-hook))))
