...
(defcustom ut-cunit-gdb-cmd-opts
  "gdb -i=mi %s"
  "GDB command options."
  :group 'ut-cunit
  :risky nil
...
(defun ut-cunit-debug-test-suite (test-suite conf)
  "Debug TEST-SUITE using path information from CONF."
  (let ((path (f-join (ut-project-dir conf) (ut-test-dir conf) (ut-test-suite-test-dir test-suite)
                            "src/" (format "%s_tests" (ut-test-suite-name test-suite)))))
    (gdb (format ut-cunit-gdb-cmd-opts path))))
...
(defun ut-cunit-create-new-test (conf test-name test-suite)
  ...
  (let* ((test-src-dir (f-join (ut-test-dir conf) (ut-test-suite-test-dir test-suite) "src"))
         ...
         (src-add-test-text (ut-format "CU_add_test(suite, \"%test-name%\", %test-name%);"
